...
## create softlinks for start and stop tomcat
echo $PATH
example: ln -s /opt/tomcat/apache/bin/startup.sh /usr/local/bin/tomcatup
         ln -s /opt/tomcat/apache/bin/startup.sh /usr/local/bin/tomcatdown
tomcatdown
tomcatup
```
#### check point :
